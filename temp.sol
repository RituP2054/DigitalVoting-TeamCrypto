...
contract Ballot {
    ...
    constructor(string[] memory proposalNames) {
        ...
        for (uint i = 0; i < proposalNames.length; i++) {
            ...
            proposalsNames.push(proposalNames[i]);
        }
    }
            ...
            returns (uint winningProposal_)
    {
        ...
        for (uint p = 0; p < proposals.length; p++) {
             if (proposals[p].voteCount == winningVoteCount) {
                tiedProposals.push(p);
             }

        }
        return tiedProposals[0];
        // winningProposal_ = tiedProposals[uint(keccak256(abi.encodePacked(block.timestamp,block.difficulty))) % numTiedProposals];
    }
    ...
    function winnerName() external
            returns (Proposal memory winnerName_)
    {
        winnerName_ = proposals[winningProposal()];
    }

    function viewProposalsName() public view returns (string[] memory){
        return proposalsNames;
    }

    function viewChairperson() public view returns (address){
        return chairperson;
    }
